...
## 评审改进项（v0.2 候选，D+3 评审讨论，不阻塞 v0.1）

**目标域名健康度管理**（赛博主审 2026-08-13 提出）：
- 现象：目标清单混入长期不可达域名（如当前网络 codeload.github.com/fastly 不可达）时，全量自检必然失败 → 每次切换都保守回滚，"宁可不变不能改坏"红线生效但切换永远无法完成
- 建议：长期不可达域名从自检集自动降级（健康度计数），核心域名（github.com/api.github.com）优先保证切换成功
- 状态：v0.2 候选，D+3（2026-08-16）评审时定方案
