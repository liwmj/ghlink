...
# v0.4.1 修复（跟进人 Linux 回归发现，2026-08-22）：control 里 Version 写死 0.2.1-1
...
# 4. 默认配置 + 示例
# v0.4.1.1（拂晓 Linux 严格测试 #3）：config.example.json 为平台无关相对路径
# （修 Windows 模板），deb 安装必须用绝对路径模板——否则状态文件从 /var/lib/ghlink/
# 漂移到 /etc/ghlink/，0.2.17→0.4.1 升级用户"失忆"（旧心跳/历史全断）。
