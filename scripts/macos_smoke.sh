...
# v0.3.0（李工 8 条③）：正常态也写 hosts 段（保证全局访问生效），
# 不再「零改动」；断言改为 ghlink 段落已写入 + 无 127.0.0.1 坏 IP
grep -q "ghlink Start" "$HOSTS" && ok "① hosts 含 ghlink 段（v0.3.0 常态写入）" || bad "① hosts 缺 ghlink 段"
...
# v0.3.0：常态刷新（periodic refresh）也会进 history，冷却期只防
