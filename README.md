...
- **开机启动**：登录时自动拉起托盘（Windows Run 键 → ghlink-tray.exe）。只管「托盘随登录出现」。
- **值守 enable**：注册定时任务（Windows schtasks，1 分钟粒度），后台持续探测 + 自愈。只管「守护跑不跑」。
- **托盘运行**：纯 UI 载体——显示状态灯 / 当前 IP / 开关按钮，**本身不做网络探测**（每 5 秒读一次状态文件刷图标）。

关键语义：**托盘开着 ≠ 值守开启**。托盘只反映 enable 状态，不自行开启值守；值守开关在托盘菜单（启用/停用值守）或命令行 `ghlink enable/disable`。安装时默认不自启（需手动勾选）。
